...
plt.imshow(np.transpose(vutils.make_grid(
    sample_batch['image'].to(device)[ : 64], padding=2, normalize=True).cpu(), (1, 2, 0)))
...
frow1 = torch.cat((torch.ones(8, 1)*color2ind['blue'], torch.ones(8, 1)*color2ind['blonde']), dim=1)
frow2 = torch.cat((torch.ones(8, 1)*color2ind['brown'], torch.ones(8, 1)*color2ind['blue']), dim=1)
frow3 = torch.cat((torch.ones(8, 1)*color2ind['red'], torch.ones(8, 1)*color2ind['black']), dim=1)
frow4 = torch.cat((torch.ones(8, 1)*color2ind['purple'], torch.ones(8, 1)*color2ind['white']), dim=1)
frow5 = torch.cat((torch.ones(8, 1)*color2ind['green'], torch.ones(8, 1)*color2ind['black']), dim=1)
frow6 = torch.cat((torch.ones(8, 1)*color2ind['bicolored'], torch.ones(8, 1)*color2ind['yellow']), dim=1)
frow7 = torch.cat((torch.ones(8, 1)*color2ind['gray'], torch.ones(8, 1)*color2ind['pink']), dim=1)
frow8 = torch.cat((torch.ones(8, 1)*color2ind['blue'], torch.ones(8, 1)*color2ind['red']), dim=1)
...
plt.show()
anim.save('anime.gif', dpi=80, writer='imagemagick')